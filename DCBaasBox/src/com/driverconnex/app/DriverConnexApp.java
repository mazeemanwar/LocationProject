...
public class DriverConnexApp extends Application {
	...
	public void onCreate() {
		...
		if (AppConfig.getIsOnlineModuleRequired().equals("yes")) {
			...
			Parse.initialize(this, AppConfig.getAppID(),
					AppConfig.getClientKey());
			...
			// --------------------------
			// Function doesn't take any params but it needs this variable
			// passed,
					...
					params, new FunctionCallback<Map<String, Object>>() {
						...
						public void done(Map<String, Object> map,
							...
							if (e == null) {
								...
								userPref.setAveragesTotal(cornering,
			...
			BaasBox.builder(this)
					...
					.setApiDomain("54.154.129.79").setPort(9000)
					.setAppCode("1234567890").init();
			userPref = new Preferences(this);
			NewRelic.withApplicationToken(AppConfig.getClientKey()).start(this);
