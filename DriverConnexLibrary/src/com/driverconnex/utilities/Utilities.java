...
public class Utilities 
{
	...
	public static boolean checkDataConnection(Context context) 
	{
	    ...
	    {
		    // Check if wifi or mobile network is available. Returns true if it is,
	...
	/**
	 * Formats number of minutes into string with format "days : hours : minutes", e.g. "1 day 5 hours 10 minutes"
	 * @param minutes
	...
	public static String formatMinutesIntoDays(long minutes)
	{
		...
		if(minutes >= 60)
		{
			...
			if(hours >= 24)
			{
				...
				// Checks if there are any minutes, if so it will display hours even if there are no hours
				if(minutesLocal > 0)
				{
				...
				{
					// Checks if there are any hours, if they are then displays hours otherwise it doesn't display them
					if(hours > 0)
					{
						builder.append(hours);
	...
	public static ArrayList<String> getEmailsFromAddressBook(Context context) 
	{
        ...
        {        
            while(cursor.moveToNext())
            {
                ...
                Cursor emailCurosr = contentResolver.query(EMAIL_CONTENT_URI, null, EMAIL_CONTACT_ID+"=?",new String[]{contact_id},null);
                
                while(emailCurosr.moveToNext())
                {
                	String email = emailCurosr.getString(emailCurosr.getColumnIndex(EMAIL));
	...
	/**
	 * Returns array of contacts from address book. Contact consists of email and full name.
	 * @return
	 */
	public static ArrayList<ArrayList<String>> getContactsFromAddressBook(Context context) 
	{
	 ...
	 * @return
	 */
	public static long calculateDuration(String d1, String t1, String d2, String t2) 
	{
		long duration = 0;
