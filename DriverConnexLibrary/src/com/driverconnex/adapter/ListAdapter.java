...
/**
 * It's a generic adapter for displaying a list of items. It's adaptable to many situations. It can display title, subtitle and icon.
 * If there is no icon specified then it will display only text. It takes ArrayList of LidAdapterItem, which is the object designed specifically for this adapter.
 * Items can be selectable, so this adapter can be used when user is selecting items on the list. Each ListAdapterItem has it's own attributes that can be expanded e.g
 * each ListAdapterItem can have it's own subtitle colour. 
 * 
 * This adapter has three attributes:
 * imageResource - a drawable resource for the icon to be displayed. If not specified then icon will not be displayed.
 ...
 *  
 * Attributes can be set at any point. They don't have to be passed to the constructor.
 *  
...
public class ListAdapter extends BaseAdapter 
{
     ...
     * @param titleColor
     */
    public ListAdapter(Context context, ArrayList<ListAdapterItem> data, int imageResource, int titleColor) 
    {
        this.data = data;
     ...
     * @param imageResource
     */
    public ListAdapter(Context context, ArrayList<ListAdapterItem> data, int imageResource) 
    {
        this.data = data;
        this.imageResource = imageResource;
        this.context = context;    
    ...
    public View getView(int position, View convertView, ViewGroup parent) 
    {
        ...
        TextView title = (TextView)convertView.findViewById(R.id.listMainTitle);
        ...
        if(imageResource != -1)
        {
        	...
        	icon.setImageDrawable(context.getResources().getDrawable(imageResource));  
        }
    	...
    	try {
    		subTitle.setTextColor(context.getResources().getColor(data.get(position).subtitleColor));
    	}
    	catch(NotFoundException e) 
    	{
    		e.printStackTrace(); 		
    		subTitle.setTextColor(context.getResources().getColor(ThemeUtilities.getMainTextColor()));
    	}      
