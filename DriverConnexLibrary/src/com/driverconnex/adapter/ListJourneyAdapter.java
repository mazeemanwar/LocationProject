...
/**
 * Adapter for display a list of journeys. It takes ArrayList of DCJourney containing information about DCJourneys and TreeSet of Integers used for displaying a separator
 * with the date. 
 * @author Yin Lee (SGI)
...
public class ListJourneyAdapter extends BaseAdapter 
{
    ...
    public ListJourneyAdapter(Context context, ArrayList<DCJourney> data, TreeSet<Integer> separatorsSet) 
    {
    	mContext = context;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) 
    {
        ...
        switch (type) 
        {
            case TYPE_ITEM:
                ...
                if (mData.get(position).getDescription() != null && mData.get(position).getDescription().length() != 0) 
                	holder.mainTitle.setText(mData.get(position).getDescription());		 
                else 
					holder.mainTitle.setText(String.valueOf(mData.get(position).getDistance()) + " Miles");
				
                holder.subTitle.setText(Utilities.formatMinutesIntoDays(mData.get(position).getDuration()));
