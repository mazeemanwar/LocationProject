...
/**
 * Activity for displaying a list of vehicle checks and items of the individual check. 
 * When user clicks on the vehicle check it will display items that belong to that check in the same activity.
...
public class VehicleChecksListActivity extends Activity 
{
	...
	private ArrayList<ListAdapterItem> adapterData = new ArrayList<ListAdapterItem>();  // Data used for the adapter
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		// We will get some extras if we launched this activity again, from vehicleChecks list to vehicleChecks
		// items list
		if (getIntent().getExtras() != null) 
		{
			items = (ArrayList<String[]>) getIntent().getExtras().getSerializable("items");
		...
		if(!displayItems)
		{
			...
			vehicleChecks = XMLVehicleChecksParser.getVehicleChecksFromXML(this);		
		}
		...
		else
		{
			...
			adapter.setImageResource(R.drawable.wrench_grey_56x56);
		}
	}
			...
			if(!DriverConnexApp.getUserPref().getMonthlyVehicleCheckExpiryDate(vehicleId).isEmpty())
			{
				...
				if(DCVehicleSingleton.isMonthlyCheckExired(vehicleId))
				{
					adapterData.get(0).subtitle = "Check expired: " + 
							DriverConnexApp.getUserPref().getMonthlyVehicleCheckExpiryDate(vehicleId);	
				}			
				else
				{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) 
	{
		if (requestCode == 100) 
		{
			...
			{
				boolean passedCheck = data.getBooleanExtra("passedCheck", false);
				int checkIndex = data.getIntExtra("checkIndex", -1);
				
				if(checkIndex != -1)
				{
					if(!adapterData.get(checkIndex).subtitle.equals("Complete") 
							&& !adapterData.get(checkIndex).subtitle.equals("Failed"))	
						countChecks ++;
					
					if(passedCheck)
					{
						adapterData.get(checkIndex).subtitle = "Complete"; 
						adapterData.get(checkIndex).subtitleColor = ThemeUtilities.getGreenTextColor();
					}
					else
					{
						adapterData.get(checkIndex).subtitle = "Failed";
						adapterData.get(checkIndex).subtitleColor = ThemeUtilities.getRedTextColor();
					}
	...
	public boolean onOptionsItemSelected(MenuItem item) 
	{
		...
		else if(item.getItemId() == R.id.action_save)
		{
			...
			{
				AlertDialog.Builder builder = new AlertDialog.Builder(VehicleChecksListActivity.this);
			...
			else
			{
				...
				DriverConnexApp.getUserPref().setMonthlyVehicleCheckExpiryDate(expiryDate, vehicleId);
				...
				query.getInBackground(vehicleId, new GetCallback<ParseObject>()
				{
					...
					public void done(ParseObject vehicle, ParseException e) 
					{
						if(e == null)
						{
							vehicle.put("vehicleLastChecked", Calendar.getInstance().getTime());
		...
		public void onItemClick(AdapterView<?> parent, View view, final int position,	long id) 
		{
			if(!displayItems)
			{
				...
				builder.setTitle("Start Check")
					   .setMessage(getResources().getString(R.string.vehicle_new_check))
					   .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() 
					   {
						   @Override
						   public void onClick(DialogInterface dialog, int which) 
						   {
								Intent intent = new Intent(VehicleChecksListActivity.this, VehicleChecksListActivity.class);				
								
								Bundle extras = new Bundle();
								extras.putSerializable("items", vehicleChecks.get(position).getItems());
			...
			{
				Intent intent = new Intent(VehicleChecksListActivity.this, VehicleCheckActivity.class);				
