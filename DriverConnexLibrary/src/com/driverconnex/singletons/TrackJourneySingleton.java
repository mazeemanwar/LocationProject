...
{	
		...
		if(!isTracking)
		{
			...
			if (GooglePlayServicesUtil.isGooglePlayServicesAvailable(context) == ConnectionResult.SUCCESS) 
			{
				if(locationClient == null)
					locationClient = new LocationClient(context, locationConnectionCallbacks, mConnectionFailedListener);
				if(recognitionClient == null)
				{
					recognitionClient =
	                new ActivityRecognitionClient(context, recognitionConnectionCallbacks, mConnectionFailedListener);
				}
				if(recognitionPendingIntent == null)
				{
					Intent intent = new Intent(context, ActivityRecognitionIntentService.class);
					
					recognitionPendingIntent =
	...
	private static LocationListener locationListener = new LocationListener() 
	{
		...
		{	
			if(journey != null)
			{
				...
				// m/s
				topSpeed = Utilities.roundTwoDecimals(location.getSpeed()) > topSpeed ? 
						Utilities.roundTwoDecimals(location.getSpeed()) : topSpeed;	
	...
	private static ConnectionCallbacks recognitionConnectionCallbacks = new ConnectionCallbacks() 
	{
	    ...
	    public void onConnected(Bundle arg0) 
	    {
	    	recognitionClient.requestActivityUpdates(DETECTION_INTERVAL_MILLISECONDS,
                    recognitionPendingIntent);
	    }
	};
	...
	/**
	 * Sets timer. Every second DCBehaviourPoint should be saved in the database.
	 */
