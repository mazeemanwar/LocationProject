...
/**
 * Activity used to display a list of all incident reports that user saved in the Parse database.
...
public class ReportListActivity extends Activity 
{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) 
	{
		if (requestCode == this.REQUEST_REPORT_POSITION) 
		{
			...
			if (resultCode == RESULT_OK) 
			{
				...
				if(position != -1)
				{
					// This item was deleted in previous activity, so update the adapter
	...
	private OnItemClickListener onItemClickListener = new OnItemClickListener() 
	{
		...
		{
			Intent intent = new Intent(ReportListActivity.this, ReviewReportActivity.class);
			intent.putExtra("report", incidents.get(position));
	...
	private void getIncidentsByParse() 
	{
		...
		{
			public void done(final List<ParseObject> incidentsList, ParseException e) 
			{
				if (e == null) 
				{					
					...
					{	
						// Make sure that this DC Incident object belongs to the user
						if(incidentsList.get(i).getParseObject("incidentUser").getObjectId()
								.contentEquals(ParseUser.getCurrentUser().getObjectId()))
						{
							// Convert parse DC Incident object into something we can read
							DCIncident incident = ParseUtilities.convertIncident(incidentsList.get(i));
					...
					// Set adapter
					adapter = new ListAdapter(ReportListActivity.this, adapterData, R.drawable.warning_grey_56x52);
